...
export default function TypeEtapeCrud() {
  // États principaux
  const [typesEtape, setTypesEtape] = useState<TypeEtape[]>([]);
  ...
  const handleViewClick = (type: TypeEtape) => {
    setCurrentType(type);
  ...
  const handleDeleteClick = (type: TypeEtape) => {
    setCurrentType(type);
  ...
  const handleAddType = async () => {
    try {
      ...
      toast.error("Erreur lors de l'ajout du type d'étape");
      console.error(error);
    }
  };
  ...
  const handleEditType = async () => {
    ...
    try {
      ...
      resetForm();
      toast.success("Type d'étape modifié avec succès");
    } catch (error) {
      toast.error("Erreur lors de la modification du type d'étape");
      console.error(error);
    }
  };
  ...
  const handleDeleteType = async () => {
    ...
    try {
      ...
      resetForm();
      toast.success("Type d'étape supprimé avec succès");
    } catch (error) {
      toast.error("Erreur lors de la suppression du type d'étape");
      console.error(error);
    }
  };
  ...
  return (
    <div className="container mx-auto py-8">
        ...
        <TabsContent value="cards">
          ...
          {loading ? (
            <div className="flex justify-center py-8">
              <p>Chargement des types d'étape...</p>
            </div>
          ) : typesEtape.length === 0 ? (
            <div className="flex justify-center py-8">
              <p>Aucun type d'étape disponible</p>
            </div>
          ) : (
      ...
      <Dialog open={openAddDialog} onOpenChange={setOpenAddDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Ajouter un type d'étape</DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Libellé du type d'étape"
                required
              />
            </div>
          </div>
      ...
      <Dialog open={openViewDialog} onOpenChange={setOpenViewDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Détails du type d'étape</DialogTitle>
          </DialogHeader>
          {currentType && (
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <div><b>Libellé :</b> {currentType?.libelle || "-"}</div>
                </div>
              </div>
            </div>
          )}
      ...
      <Dialog open={openEditDialog} onOpenChange={setOpenEditDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Modifier le type d'étape</DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="libelle">Libellé</Label>
